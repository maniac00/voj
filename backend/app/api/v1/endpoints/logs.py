...
async def backup_logs(
    ...
    if settings.ENVIRONMENT != "local":
        raise HTTPException(
            status_code=400, detail="Log backup is only supported in local environment"
        )
    ...
    try:
        ...
        backup_data = {
            "backup_info": {
                ...
                "end_time": backup_request.end_time.isoformat()
                if backup_request.end_time
                else None,
        ...
        raise HTTPException(status_code=500, detail=f"Log backup failed: {str(e)}")


@router.get("/backups")
async def list_log_backups(
    limit: int = Query(50, ge=1, le=100), claims=Depends(require_any_scope(["admin"]))
):
    """
    저장된 로그 백업 목록 조회
    """
...
async def delete_log_backup(
    backup_filename: str, claims=Depends(require_any_scope(["admin"]))
):
    """
    로그 백업 파일 삭제
    """
