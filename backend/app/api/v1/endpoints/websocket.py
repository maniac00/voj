...
from app.services.websocket.log_streamer import LogCategory, LogLevel, log_streamer
...
async def handle_client_message(connection_id: str, message: dict) -> None:
    ...
    if message_type == "subscribe":
        ...
        # 연결 확인
        await log_streamer.send_to_connection(
...
async def websocket_chapter_status(
    websocket: WebSocket, chapter_id: str, connection_id: Optional[str] = Query(None)
):
    """
...
async def send_current_chapter_status(connection_id: str, chapter_id: str) -> None:
    """현재 챕터 상태 전송 (SQL 우선, 실패 시 DynamoDB 모델로 폴백)"""

    def _extract_file_name(audio_key: Optional[str]) -> Optional[str]:
        if not audio_key:
            return None
    ...
    try:
        ...
        with SessionLocal() as db:
            ...
            if row is not None:
                status_value = "ready"  # SQL 모델에는 상태 필드가 없어 실사용 상태로 고정
                status_data = {
                    "type": "chapter_status",
                    "chapter_id": chapter_id,
                    # 신규 키("status")와 구키("chapter_status")를 모두 제공해 클라이언트 호환
                    "status": status_value,
                    "chapter_status": status_value,
        ...
        # 2) 폴백: DynamoDB(PynamoDB) 모델 조회 (레거시 환경용)
        try:
            from app.models.audio_chapter import AudioChapter  # type: ignore

            chapter = AudioChapter.get_by_id(chapter_id)
            if not chapter:
                await log_streamer.send_to_connection(
                    connection_id,
                    {"type": "error", "message": f"Chapter {chapter_id} not found"},
                )
                return

            status_value = chapter.status or "ready"
            status_data = {
                "type": "chapter_status",
                "chapter_id": chapter_id,
                "status": status_value,
        ...
        await log_streamer.send_to_connection(
            connection_id,
            {"type": "error", "message": f"Failed to get chapter status: {str(e)}"},
        )
