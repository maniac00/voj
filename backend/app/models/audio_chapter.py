...
class AudioChapter(BaseModel):
    ...
    class Meta:
        table_name = getattr(
            settings, "AUDIO_CHAPTERS_TABLE_NAME", "voj-audio-chapters"
        )
    ...
    status = UnicodeAttribute(
        default="uploading", attr_name="status"
    ...
    # 처리 정보
    processing_started_at = UTCDateTimeAttribute(
        null=True, attr_name="processing_started_at"
    )
    processing_completed_at = UTCDateTimeAttribute(
        null=True, attr_name="processing_completed_at"
    )
    ...
    def list_by_book(cls, book_id: str, limit: int = 50):
        ...
        return cls.book_chapters_index.query(
            book_id, limit=limit, scan_index_forward=True  # 챕터 번호 오름차순
        )
    ...
    def list_by_status(cls, status: str, limit: int = 10):
        ...
        return cls.status_index.query(
            status, limit=limit, scan_index_forward=False  # 최신순
        )
    ...
    def get_next_chapter_number(cls, book_id: str) -> int:
        ...
        try:
            chapters = list(
                cls.book_chapters_index.query(
                    book_id, scan_index_forward=False, limit=1  # 최신 챕터부터
                )
            )
    ...
    def to_dict(self) -> dict:
        ...
        if self.file_info:
            result["file_info"] = {
                ...
                "s3_key": self.file_info.s3_key
                if hasattr(self.file_info, "s3_key")
                else None,
                "local_path": self.file_info.local_path
                if hasattr(self.file_info, "local_path")
                else None,
            }
        ...
        if self.audio_metadata:
            result["audio_metadata"] = {
                ...
                if hasattr(self.audio_metadata, "bitrate")
                else None,
                "sample_rate": self.audio_metadata.sample_rate
                if hasattr(self.audio_metadata, "sample_rate")
                else None,
                "channels": self.audio_metadata.channels
                if hasattr(self.audio_metadata, "channels")
                else None,
                "format": self.audio_metadata.format
                if hasattr(self.audio_metadata, "format")
                else None,
            }
