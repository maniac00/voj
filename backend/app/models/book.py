...
class Book(BaseModel):
    ...
    class Meta:
        table_name = getattr(settings, "BOOKS_TABLE_NAME", "voj-books")
    ...
    status = UnicodeAttribute(
        default="draft", attr_name="status"
    )  # draft, processing, published, error
    ...
    @classmethod
    def list_by_user(
        cls, user_id: str, limit: int = 10, last_evaluated_key: dict = None
    ):
        """사용자의 책 목록 조회"""
        query_kwargs = {"limit": limit, "scan_index_forward": False}  # 최신순 정렬
    ...
    def list_by_user_and_status(cls, user_id: str, status: str, limit: int = 10):
        ...
        return cls.status_index.query(
            user_id, cls.status == status, limit=limit, scan_index_forward=False
        )
    ...
    def list_by_user_and_genre(cls, user_id: str, genre: str, limit: int = 10):
        ...
        return cls.genre_index.query(
            user_id, cls.genre == genre, limit=limit, scan_index_forward=False
        )
