...
class WebSocketLogStreamer:
    ...
    async def unsubscribe_from_chapter(
        self, connection_id: str, chapter_id: str
    ) -> None:
        """챕터 로그 구독 해제"""
        with self.lock:
            if connection_id in self.subscriptions:
    ...
    async def broadcast_log(self, log_message: LogMessage) -> None:
        ...
        # 관련 연결들에게 전송
        message_data = {"type": "log", "data": log_message.to_dict()}
        ...
        with self.lock:
            for connection_id, subscribed_chapters in self.subscriptions.items():
                ...
                if not subscribed_chapters or (
                    log_message.chapter_id
                    and log_message.chapter_id in subscribed_chapters
                ):
...
def setup_websocket_logging() -> None:
    ...
    loggers_to_stream = ["app.api.v1.endpoints.files", "app.api.v1.endpoints.audio"]
