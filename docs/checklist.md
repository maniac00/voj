...
- [ ] **6.1.2** 스택별 리소스 정의
  ...
  - [ ] **6.1.2.3** ApiStack (ECR + ECS Fargate + ALB)
  - [ ] **6.1.2.4** AuthStack (Cognito + IAM)
- [ ] **6.1.3** 환경별 설정 분리 (dev/staging/prod)

### 6.2 배포 스크립트 구성
- [ ] **6.2.1** `backend/Dockerfile` 작성 및 로컬 빌드 확인
- [ ] **6.2.2** GitHub Actions: ECR 로그인, 빌드/푸시, ECS 서비스 업데이트 파이프라인

### 6.3 모니터링 및 알람 설정
- [ ] **6.3.1** CloudWatch 로그 그룹 생성
- [ ] **6.3.2** 주요 메트릭 알람 설정
  - [ ] **6.3.2.1** ECS 서비스 Unhealthy Target 수 알람
...
- main 병합 준비: AWS 배포 경로 제거, Railway/Vercel 기준으로 워크플로우/설정 정리, 첫 MVP 배포 시도
...
- Lambda/APIGW 배포 스크립트에 deprecated 헤더 추가
