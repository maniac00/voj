...
import { getChapters } from "@/lib/audio";
...
export default function BooksPage() {
      ...
      try {
        const entries = await Promise.all(
          needsCheck.map(async (b) => {
            try {
              const cs = await getChapters(b.book_id);
              return [b.book_id, cs.length] as const;
            } catch {
              return [b.book_id, 0] as const;
            }
          }),
        );
        if (!cancelled) {
  ...
  const handleDeleteBook = async () => {
    ...
    try {
      ...
      // 목록에서 삭제된 책 제거
      setBooks((prev) =>
        prev.filter((book) => book.book_id !== bookToDelete.book_id),
      );
            ...
            <thead className="bg-gray-50">
              <tr role="row">
                ...
                <AccessibleTableHeader
                  sortable={true}
                  sortDirection={
                    sortField === "publisher" ? sortDirection : null
                  }
                ...
                <AccessibleTableHeader>장르</AccessibleTableHeader>
                ...
                <AccessibleTableHeader>챕터</AccessibleTableHeader>

                <AccessibleTableHeader
                  sortable={true}
                  sortDirection={
                    sortField === "created_at" ? sortDirection : null
                  }
                ...
                <AccessibleTableHeader>작업</AccessibleTableHeader>
              </tr>
            </thead>
