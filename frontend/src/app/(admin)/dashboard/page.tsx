...
export default function AdminDashboardPage() {
  ...
  const [recent, setRecent] = useState<
    Array<{ book: BookDto; chapter: ChapterDto }>
  >([]);
    ...
    const load = async () => {
      try {
        ...
        if (bookList && bookList.length > 0) {
          ...
          recentList.sort((a, b) => {
            const tA = Date.parse(
              a.chapter.updated_at || a.chapter.created_at || "1970-01-01",
            );
            const tB = Date.parse(
              b.chapter.updated_at || b.chapter.created_at || "1970-01-01",
            );
            return tB - tA;
          });
              ...
              >
                <div className="min-w-0">
                  ...
                  <div className="truncate text-gray-500">
                    {book.title} • 챕터 {chapter.chapter_number} •{" "}
                    {(chapter.file_size / (1024 * 1024)).toFixed(1)}MB
                  </div>
                </div>
                <div className="text-gray-500 ml-4 whitespace-nowrap">
                  {new Date(
                    chapter.updated_at || chapter.created_at || Date.now(),
                  ).toLocaleString()}
                </div>
              </li>
            ))}
