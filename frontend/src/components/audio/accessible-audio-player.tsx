...
export function AccessibleAudioPlayer({
  ...
  // 음성 안내 함수들
  const announcePlayState = useCallback(
    (playing: boolean) => {
  ...
  const announceTimeJump = useCallback(
    (seconds: number, direction: "forward" | "backward") => {
  ...
  const announceSpeedChange = useCallback(
    (speed: number) => {
  ...
  const announceTimePosition = useCallback(
  ...
  return (
    ...
    >
      ...
      <div id="player-help" className="sr-only">
        오디오 플레이어입니다. 스페이스바로 재생 일시정지, 화살표 키로 탐색 및
        볼륨 조절, H키로 전체 도움말을 들을 수 있습니다.
      </div>
      ...
      <div className="mb-4">
        ...
        <div className="text-sm text-gray-600" aria-live="polite">
          {isPlaying ? "재생 중" : "일시정지됨"} • 재생 속도: {playbackRate}x •
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              setCurrentTime(newTime);
              announce(
                `${Math.floor(newTime / 60)}분 ${Math.floor(newTime % 60)}초로 이동`,
              );
            }}
          ...
          <div className="flex justify-between mt-1 text-sm text-gray-600">
            <span
              aria-label={`현재 시간 ${Math.floor(currentTime / 60)}분 ${Math.floor(currentTime % 60)}초`}
            >
            ...
            <span
              aria-label={`전체 시간 ${Math.floor(audioDuration / 60)}분 ${Math.floor(audioDuration % 60)}초`}
            >
      ...
      <details className="mt-4 border-t border-gray-200 pt-4">
        ...
        <div className="mt-2 text-sm text-gray-600 space-y-1">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            <div>
              <kbd className="bg-gray-100 px-1 rounded">스페이스</kbd>{" "}
              재생/일시정지
            </div>
            <div>
              <kbd className="bg-gray-100 px-1 rounded">←/→</kbd> 10초 건너뛰기
            </div>
            <div>
              <kbd className="bg-gray-100 px-1 rounded">↑/↓</kbd> 볼륨 조절
            </div>
            <div>
              <kbd className="bg-gray-100 px-1 rounded">M</kbd> 음소거 토글
            </div>
            <div>
              <kbd className="bg-gray-100 px-1 rounded">I</kbd> 현재 위치 안내
            </div>
            <div>
              <kbd className="bg-gray-100 px-1 rounded">1-5</kbd> 위치 이동 (20%
              단위)
            </div>
            <div>
              <kbd className="bg-gray-100 px-1 rounded">+/-</kbd> 재생 속도 조절
            </div>
            <div>
              <kbd className="bg-gray-100 px-1 rounded">H</kbd> 도움말
            </div>
          </div>
        </div>
      </details>
      ...
      <div className="sr-only">
        <AudioPlayer
          ...
          onLoadedMetadata={(dur) => {
            ...
            announce(
              `${title} 로드됨. 재생시간 ${Math.floor(dur / 60)}분 ${Math.floor(dur % 60)}초`,
            );
          }}
          volume={volume}
          muted={isMuted}
...
export function AccessiblePlaylist({
  ...
  const currentIndex = readyChapters.findIndex(
              ...
              <div className="flex items-center justify-between">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500">
                    ...
                    {Math.floor(chapter.duration % 60)}초
                  </p>
                </div>
