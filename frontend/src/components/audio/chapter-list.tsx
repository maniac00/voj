...
export function ChapterList({
  ...
  const [chapterToDelete, setChapterToDelete] = useState<ChapterDto | null>(
    null,
  );
    ...
    return (
      ...
      >
        ...
        <p className="text-gray-500 mb-4">
          아직 업로드된 오디오 파일이 없습니다.
        </p>
        <p className="text-sm text-gray-400">
          오디오 파일을 업로드하여 첫 번째 챕터를 만들어보세요.
        </p>
      </div>
    );
  }
                      ...
                      <div className="flex items-center">
                        <div className="animate-pulse h-5 w-5 flex items-center justify-center">
                          <div className="grid grid-cols-3 gap-0.5">
                            <div
                              className="w-1 bg-green-600 animate-bounce"
                    ...
                    onClick={async () => {
                      try {
                        const response = await fetch(
                          `/api/v1/files/retry-processing/${chapter.chapter_id}`,
                          {
                            method: "POST",
                            headers: {
                        ...
                        if (response.ok) {
                          ...
                          const errorData = await response
                            .json()
                            .catch(() => ({ detail: "Reprocessing failed" }));
                          throw new Error(errorData.detail);
                        }
                      } catch (error) {
                        showError(
                          `재처리 실패: ${error instanceof Error ? error.message : "알 수 없는 오류"}`,
                        );
                      }
                    }}
...
export function ChapterStats({ chapters, className = "" }: ChapterStatsProps) {
  ...
  const totalSize = chapters.reduce(
    (sum, chapter) => sum + chapter.file_size,
    0,
  );
    ...
    if (term) {
      ...
      filtered = filtered.filter(
        (chapter) =>
          ...
          (chapter.description &&
            chapter.description.toLowerCase().includes(lowerTerm)),
      );
    }
