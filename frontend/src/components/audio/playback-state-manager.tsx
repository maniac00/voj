...
export function PlaybackStateManager({
  ...
  const handleResumeFromHistory = (record: PlaybackHistory) => {
    ...
    info(
      `${record.chapterTitle} ${formatTime(record.currentTime)} 위치에서 재생을 재개합니다.`,
    );
  };
...
export function PlaybackStats({ bookId, className = "" }: PlaybackStatsProps) {
  const { history, getCompletedChapters, getInProgressChapters } =
    usePlaybackHistory(bookId);
      ...
      {history.length > 0 && (
        <div className="mt-4 pt-4 border-t border-gray-200">
          <div className="text-xs text-gray-500 text-center">
            마지막 활동:{" "}
            {formatDate(
              new Date(Math.max(...history.map((h) => h.playedAt.getTime()))),
            )}
          </div>
        </div>
      )}
