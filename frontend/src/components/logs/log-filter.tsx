...
export function LogFilter({
  ...
  const [savedFilters, setSavedFilters] = useState<
    Array<{ name: string; filter: LogFilter }>
  >([]);
  ...
  const uniqueChapterIds = [
    ...new Set(logs.map((log) => log.chapter_id).filter(Boolean)),
  ];
  const uniqueJobIds = [
    ...new Set(logs.map((log) => log.job_id).filter(Boolean)),
  ];
  ...
  useEffect(() => {
    ...
    if (filter.chapterIds.length > 0) {
      filtered = filtered.filter(
        (log) => log.chapter_id && filter.chapterIds.includes(log.chapter_id),
      );
    }
    ...
    if (filter.jobIds.length > 0) {
      filtered = filtered.filter(
        (log) => log.job_id && filter.jobIds.includes(log.job_id),
      );
    }
    ...
    <div className={`bg-white rounded-lg border border-gray-200 ${className}`}>
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            onClick={() =>
              updateFilter({ showOnlyErrors: !filter.showOnlyErrors })
            }
          ...
          <button
            onClick={() =>
              updateFilter({ showOnlyRecent: !filter.showOnlyRecent })
            }
            className={`text-sm px-3 py-1 rounded-full border ${
                  ...
                  {uniqueChapterIds.map((chapterId) => (
                    ...
                    >
                      <input
                        ...
                        checked={
                          chapterId
                            ? filter.chapterIds.includes(chapterId as string)
                            : false
                        }
                        onChange={(e) => {
          ...
          <div className="flex items-center justify-between text-sm text-gray-600">
            <div>
              {getActiveFilterCount() > 0 && (
                <span>{getActiveFilterCount()}개 필터 활성화</span>
              )}
            </div>
            <div>
              {logs.length > 0 && (
                <span>{logs.length}개 로그 중 필터 적용 결과 표시</span>
              )}
            </div>
          </div>
        </div>
...
export function LogSearch({ logs, onSearch, className = "" }: LogSearchProps) {
  ...
  const [searchType, setSearchType] = useState<"simple" | "regex" | "advanced">(
    "simple",
  );
        ...
        {searchType === "advanced" && (
          <div className="bg-blue-50 border border-blue-200 rounded p-3 text-sm text-blue-700">
            ...
            <div className="space-y-1 text-xs">
              <div>
                <code>level:error</code> - 에러 레벨 로그만
              </div>
              <div>
                <code>category:encoding</code> - 인코딩 카테고리만
              </div>
              <div>
                <code>message:upload</code> - 메시지에 'upload' 포함
              </div>
              <div>
                <code>chapter:abc123</code> - 특정 챕터 ID
              </div>
            </div>
          </div>
        )}
