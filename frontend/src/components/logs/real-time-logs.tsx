...
import {
  ErrorAnalyzer,
  QuickErrorSolution,
} from "@/components/error/error-analyzer";
...
export function RealTimeLogs({
  ...
  const { logs, connectionStatus, error, connect, disconnect, clearLogs } =
    useWebSocketLogs({
      chapterId,
      autoConnect: true,
      maxLogHistory: 200,
  ...
  const [autoScrollState, setAutoScrollState] =
    useState<boolean>(autoScrollProp);
  ...
  return (
    <div className={`bg-white rounded-lg border border-gray-200 ${className}`}>
      ...
      <div className="px-4 py-3 border-b border-gray-200">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <span
              className={`inline-flex items-center px-2 py-1 text-xs font-medium rounded-full ${getConnectionStatusColor()}`}
            >
              <div
                className={`w-2 h-2 rounded-full mr-1 ${connectionStatus === "connected" ? "bg-green-500" : connectionStatus === "connecting" ? "bg-yellow-500 animate-pulse" : "bg-red-500"}`}
              ></div>
      ...
      {/* 간단 검색 */}
      {false && !showAdvancedFilters && (
        <LogSearch logs={logs} onSearch={setFilteredLogs} />
      )}
            ...
            {filteredLogs.map((log) => (
              <div key={log.id} className="px-4 py-2 hover:bg-gray-50">
                <div className="flex items-start space-x-3">
                  {/* 아이콘 및 레벨 */}
                  <div
                    className={`flex-shrink-0 inline-flex items-center px-2 py-1 text-xs font-medium rounded ${getLogLevelColor(log.level)}`}
                  >
