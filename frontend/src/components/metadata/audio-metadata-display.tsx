...
export function AudioMetadataDisplay({
  ...
  const getQualityInfo = (bitrate: number, channels: number) => {
    if (bitrate >= 320)
      return { level: "excellent", text: "최고 품질", color: "text-green-600" };
    if (bitrate >= 192)
      return { level: "high", text: "고품질", color: "text-blue-600" };
    if (bitrate >= 128)
      return { level: "medium", text: "중간 품질", color: "text-yellow-600" };
    if (bitrate >= 64)
      return { level: "medium", text: "표준 품질", color: "text-gray-600" };
        ...
        <div className="space-y-3">
          ...
          <div className="flex justify-between items-center">
            ...
            <span className="text-sm font-medium">
              {getChannelText(metadata.channels)}
            </span>
          </div>
          ...
          <div className="flex justify-between items-center">
            ...
            <span className="text-sm font-medium">
              {metadata.sample_rate.toLocaleString()} Hz
            </span>
          </div>
              ...
              <div className="mt-3 space-y-2 text-sm text-gray-600">
                ...
                {metadata.file_size && metadata.duration > 0 && (
                  <div className="flex justify-between">
                    <span>실제 비트레이트</span>
                    <span>
                      {calculateBitrate(metadata.file_size, metadata.duration)}
                      kbps
                    </span>
                  </div>
                )}
      ...
      <div className="grid grid-cols-3 gap-4 text-xs">
        <div className="text-center">
          ...
          <div className="space-y-1">
            ...
            {original.file_size && (
              <div>{formatFileSize(original.file_size)}</div>
            )}
          </div>
        </div>
        ...
        <div className="text-center">
          ...
          <div className="space-y-1">
            ...
            {processed.file_size && (
              <div>{formatFileSize(processed.file_size)}</div>
            )}
          </div>
        </div>
      </div>
...
export function RealTimeMetadata({
  chapterId,
  className = "",
}: RealTimeMetadataProps) {
    ...
    const fetchMetadata = async () => {
      try {
        ...
        const response = await fetch(
          `/api/v1/audio/${chapterId.split("/")[0]}/chapters/${chapterId}`,
          {
            headers: {
        ...
        <div className="text-xs text-blue-700">
          ...
          <div className="space-y-1">
            {estimatedMetadata.duration && (
              <div>
                재생시간: ~{formatDurationTextLocal(estimatedMetadata.duration)}
              </div>
            )}
