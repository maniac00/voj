...
import {
  ...
  MetadataProgress,
} from "@/components/metadata/audio-metadata-display";
    ...
    <div className={`bg-white border border-gray-200 rounded-lg ${className}`}>
      ...
      <div className="p-4 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span
            className={`inline-flex px-3 py-1 text-sm font-medium rounded-full border ${getStatusColor(status.chapter_status)}`}
          >
          ...
          <div className="bg-gray-50 rounded-lg p-3">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`inline-flex px-2 py-1 text-xs font-medium rounded ${getStatusColor(status.encoding_job.status)}`}
                >
              ...
              {status.encoding_job.progress > 0 && (
                <div>
                  <div className="flex items-center justify-between text-xs text-gray-600 mb-1">
                    <span>진행률</span>
                    <span>
                      {Math.round(status.encoding_job.progress * 100)}%
                    </span>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div
                      ...
                      style={{
                        width: `${status.encoding_job.progress * 100}%`,
                      }}
                    />
                  </div>
                </div>
        ...
        {status.metadata ? (
          ...
          <MetadataProgress
            ...
            currentStep={
              status.encoding_job.progress < 0.3
                ? "파일 검증 중..."
                : status.encoding_job.progress < 0.5
                  ? "인코딩 준비 중..."
                  : status.encoding_job.progress < 0.8
                    ? "오디오 변환 중..."
                    : "메타데이터 추출 중..."
            }
          />
  ...
  const getStatusIcon = () => {
    switch (status) {
      ...
      case "uploading":
        return (
          <div
            className={`animate-spin rounded-full border-2 border-blue-200 border-t-blue-500 ${sizeClasses[size]}`}
          ></div>
        );
      case "error":
        return (
      ...
      default:
        return (
          <div
            className={`rounded-full bg-gray-300 ${sizeClasses[size]}`}
          ></div>
        );
    }
  };
