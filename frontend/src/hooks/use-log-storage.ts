...
export function useLogStorage() {
  ...
  const startSession = useCallback(
    (name: string, chapterId?: string, bookId?: string) => {
  ...
  const exportLogs = useCallback(
    (logs: LogMessage[], options: LogExportOptions, filename?: string) => {
      try {
        ...
        if (options.dateRange) {
          filteredLogs = filteredLogs.filter((log) => {
            ...
            return (
              logDate >= options.dateRange!.start &&
              logDate <= options.dateRange!.end
            );
          });
        }
        ...
        link.download =
          filename ||
          `logs_${new Date().toISOString().slice(0, 19).replace(/:/g, "-")}.${options.format}`;
  ...
  const exportSession = useCallback(
    (sessionId: string, options: LogExportOptions) => {
