...
export function usePlaybackState(
  ...
  const cleanupOldStates = useCallback(() => {
    try {
      ...
      // 오래된 것부터 삭제
      const toDelete = statesWithTime.slice(
        0,
        keys.length - config.maxSavedStates,
      );
  ...
  const createBookmark = useCallback(
    (name?: string) => {
      const bookmarkName =
        name ||
        `${Math.floor(state.currentTime / 60)}분 ${Math.floor(state.currentTime % 60)}초`;
...
export function usePlaybackHistory(bookId: string) {
    ...
    (
      ...
      try {
        const updatedHistory = [
          newRecord,
          ...history.filter((h) => h.chapterId !== chapterId),
  ...
  const getInProgressChapters = useCallback(() => {
    return history.filter(
      (h) => h.completedPercentage >= 5 && h.completedPercentage < 95,
    );
  }, [history]);
...
export function usePlaybackSession(bookId: string) {
  ...
  const endSession = useCallback(() => {
    if (currentSession) {
      ...
      try {
        ...
        const updatedSessions = [endedSession, ...existingSessions].slice(
          0,
          20,
        ); // 최대 20개 세션 유지
  ...
  // 챕터 재생 기록
  const recordChapterPlay = useCallback(
