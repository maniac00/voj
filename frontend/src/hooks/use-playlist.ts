...
export function usePlaylist({
  ...
  const goToChapter = useCallback(
    (chapterId: string) => {
      ...
      const playOrderIndex = playOrder.findIndex(
  ...
  const getPlaylistStats = useCallback(() => {
    ...
    return {
      ...
      remainingDuration:
        state.totalDuration - (state.playedDuration + state.currentTime),
      ...
      estimatedTimeToEnd: currentChapter
        ? currentChapter.duration -
          state.currentTime +
          state.items
            .slice(playOrder.indexOf(state.currentIndex) + 1)
