...
export function useUploadErrorHandler() {
  ...
  const parseUploadError = useCallback(
    (error: Error | string, context?: any): UploadError => {
  ...
  const getRetryDelay = useCallback(
    (fileId: string, config: RetryConfig = DEFAULT_RETRY_CONFIG): number => {
      const attempts = retryAttempts.get(fileId) || 0;
