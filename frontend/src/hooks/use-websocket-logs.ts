...
export function useWebSocketLogs({
  ...
  const subscribeToChapter = useCallback(
    (chapterId: string) => {
      sendMessage({
        type: "subscribe",
  ...
  const unsubscribeFromChapter = useCallback(
    (chapterId: string) => {
      sendMessage({
        type: "unsubscribe",
        chapter_id: chapterId,
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      ws.onerror = (error) => {
        ...
        try {
          const ready = (ws as any)?.readyState;
          console.error("Chapter status WebSocket error:", {
            error,
            url: ws.url,
            readyState: ready,
          });
  ...
  useEffect(() => {
    if (connectionStatus === "disconnected" && chapterId && pollInterval > 0) {
      const interval = setInterval(() => {
        // HTTP API로 상태 조회 (폴백)
        fetch(
          `/api/v1/audio/${encodeURIComponent(chapterId.split("/")[0])}/chapters/${encodeURIComponent(chapterId)}`,
        )
          .then((res) => res.json())
          .then((data) => {
            // 간단한 상태 정보만 업데이트
            setStatus((prev) =>
              prev ? { ...prev, chapter_status: data.status } : null,
            );
          })
          .catch(() => {
