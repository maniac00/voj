    ...
    if (metadata) {
      ...
      if (typeof metadata.duration === "number") {
        if (opts.minDuration && metadata.duration < opts.minDuration) {
          errors.push(
            `재생시간이 너무 짧습니다. 최소: ${opts.minDuration}초, 현재: ${Math.round(metadata.duration)}초`,
          );
        }

        if (opts.maxDuration && metadata.duration > opts.maxDuration) {
          errors.push(
            `재생시간이 너무 깁니다. 최대: ${Math.round((opts.maxDuration || 0) / 60)}분, 현재: ${Math.round(metadata.duration / 60)}분`,
          );
        }
      }
      ...
      // 샘플레이트 검증 (안전 가드)
      if (
        opts.allowedSampleRates &&
        typeof metadata.sampleRate === "number" &&
        !opts.allowedSampleRates.includes(metadata.sampleRate)
      ) {
      ...
      if (
        ...
        // 1시간 이상
        warnings.push(
          "긴 오디오 파일입니다. 업로드에 시간이 오래 걸릴 수 있습니다.",
        );
      }
    }
  } catch (error) {
...
export function analyzeAudiobookSeries(files: File[]): {
  ...
  const duplicateNumbers = chapterNumbers.filter(
    (num, index) => chapterNumbers.indexOf(num) !== index,
  );

  if (duplicateNumbers.length > 0) {
    warnings.push(
      `중복된 챕터 번호가 있습니다: ${duplicateNumbers.join(", ")}`,
    );
  }
 ...
 */
export function quickValidateAudioFile(file: File): {
  isValid: boolean;
  error?: string;
} {
