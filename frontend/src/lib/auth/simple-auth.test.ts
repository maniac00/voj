...
describe("Simple Auth Utilities", () => {
  ...
  describe("login", () => {
    it("should successfully login with valid credentials", async () => {
      ...
      const result = await login({ username: "admin", password: "qwer1234" });
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        "http://localhost:8000/api/v1/auth/login",
        {
          ...
          body: JSON.stringify({ username: "admin", password: "qwer1234" }),
        },
      );
    ...
    it("should handle network errors", async () => {
      ...
      await expect(
        login({ username: "admin", password: "qwer1234" }),
  ...
  describe("server-side rendering", () => {
    it("should handle absence of window object", () => {
      ...
      // @ts-expect-error - deleting window on global for SSR simulation
